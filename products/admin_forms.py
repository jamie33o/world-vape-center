...
class AdminAddMultiOptionForm(forms.Form):
    """
    Form for adding multiple options to
    selected products in the admin interface.

    Attributes:
    - ids (CharField): Hidden input field for
    storing selected product IDs.
    - options_name (CharField): Field for
    specifying the options name.
    - choices (MultipleChoiceField):
    Multiple choice field for selecting options.
    ...
    def __init__(self, *args, **kwargs):
        ...
        choices_list = [(option.id, option.name)
                        for option in multi_option_data]
