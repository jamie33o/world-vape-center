...
class ProductSitemap(Sitemap):
    ...
    Attributes:
    - changefreq (str): The anticipated frequency of changes to the content ('monthly').
    - priority (float): The priority of the content in relation to other content (0.9).

    Methods:
    - items(): Returns the queryset of products to be included in the sitemap.
    - location(item): Returns the URL location for a given product in the sitemap.
...
class CategorySitemap(Sitemap):
    ...
    Methods:
    - items(): Returns the queryset of categories to be included in the sitemap.
    - location(item): Returns the URL location for a given category in the sitemap.
